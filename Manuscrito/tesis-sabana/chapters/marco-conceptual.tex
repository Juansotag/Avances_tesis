...
La principal ventaja matemática y operativa de este enfoque frente a arquitecturas específicas de campaña como el SOMEN-DC \parencite{brito2023somen} radica en su escalabilidad e independencia. Al abstraer la función $f$ sobre $M$ elecciones, el modelo se desacopla de la necesidad de contar con datos de encuestas como variable dependiente, utilizando directamente los resultados electorales finales como \textit{ground truth}. Asimismo, elimina la dependencia de infraestructura de \textit{scraping} en tiempo real, pues la predicción sobre una elección nueva $\mathcal{E}_{new}$ requiere únicamente extraer los vectores $\mathbf{x}_{new}$ y procesarlos a través del modelo pre-entrenado.
...
El uso de señales digitales como estimador político se sustenta empíricamente en el trabajo fundacional de \textcite{tumasjan2010} sobre las elecciones federales alemanas de 2009. Estos autores demostraron que el simple conteo volumétrico de \textit{tweets} que mencionan a un partido refleja fielmente el resultado electoral, logrando un error absoluto medio (MAE) de 1{,}65\%. Esta hipótesis sugiere que la atención (reflejada en interacciones y contenido generado) equivale a capital político en la arena digital.
...
No obstante, conviene precisar el alcance inferencial de este \textit{proxy}. ELA-NOM no mide intención de voto en sentido estricto, una variable actitudinal y psicológica que requeriría sondeo directo al elector, sino que aproxima la \textbf{cuota electoral observada} a partir de señales digitales observables. Además del apoyo electoral genuino, las interacciones en redes sociales pueden reflejar: (i)~\textbf{visibilidad mediática}, cuando un candidato es cubierto ampliamente por medios que generan reacciones digitales sin que ello equivalga a apoyo; (ii)~\textbf{polarización y rechazo}, ya que las interacciones negativas (comentarios críticos, \textit{shares} de denuncia) también incrementan los conteos de engagement; (iii)~\textbf{pauta publicitaria paga}, que infla artificialmente las métricas mediante contenido promovido; y (iv)~\textbf{militancia digital organizada} (posible actividad coordinada no orgánica, incluyendo bots y \textit{astroturfing}). Las redes sociales son ecosistemas permeables a estos fenómenos, así como a los sesgos demográficos propios de cada plataforma y a la existencia de candidatos cuya tracción digital es desproporcionada a su viabilidad electoral real (como se evidenció con la exclusión de Gustavo Bolívar en el dataset colombiano).
...
\noindent donde $t$ es el tiempo transcurrido (en días) desde la publicación; $k$ es el parámetro de tasa que determina la velocidad general de acumulación; $\alpha$ es el parámetro de forma que controla la geometría de la curva de crecimiento acumulado (valores $\alpha < 1$ producen curvas cóncavas con desaceleración progresiva; valores $\alpha > 1$ producen curvas convexas con tasa de acumulación creciente, como ocurre con Twitter/X, calibrado con $\alpha = 1{,}1422$; $\alpha = 1$ recupera el modelo exponencial simple, descartado por ajuste inferior); y $t_0 > 0$ adelanta el origen de la acumulación, haciendo que $F(0) > 0$ y capturando la explosión masiva de interacciones que ocurre en las primeras horas antes de que $t$ alcance el primer día completo.
